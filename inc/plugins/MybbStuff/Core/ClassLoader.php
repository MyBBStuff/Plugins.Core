...
class MybbStuff_Core_ClassLoader
{
	 ...
	 * 
	 * @param string $nameSpace The namespace to add (in the form "MybbStuff_SimpleLikes").
	...
	public function registerNamespace($nameSpace = '', $paths = '')
	{
		...
		if (!isset($this->registeredNamespaces[$nameSpace])) {			
			...
			$this->registeredNamespaces[$nameSpace] = array_merge($this->registeredNamespaces[$nameSpace], $paths);
		}
	}
		...
		if (count($classParts) >= 2) {
			...
			if (isset($this->registeredNamespaces[$baseNameSpace])) {
				...
				foreach ($this->registeredNamespaces[$baseNameSpace] as $dir) {
					$classPath = $dir . DIRECTORY_SEPARATOR . implode(DIRECTORY_SEPARATOR, $classParts) . DIRECTORY_SEPARATOR . $className . '.php';
